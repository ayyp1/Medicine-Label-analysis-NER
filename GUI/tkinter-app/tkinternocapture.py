...
my_w = tk.Tk()
my_w.iconbitmap("videos/tkinter-app/logo.ico")
my_w.geometry("1000x800")  # Size of the window 
my_w.title('MEDICAL NER')
Title_font=('Ubuntu', 20, 'bold')
Normal_font=('Ubuntu' , 14 )
scroll_font= ('Ubuntu' , 10)

#background image 
bg = ImageTk.PhotoImage(file = "Videos/tkinter-app/pharms.jpg") 
...
def app(click):
    ...
    save_folder = 'Videos/tkinter-app/uploaded_images/'
    ...
    image_label.grid( row = 1, column=1 , pady=50 , padx = 200 )
    
    #apply ocr 
    result = ocr_model.ocr(os.path.join('Videos/tkinter-app/uploaded_images/med.jpg'))
    output=" "
    for res in result:
     for re in res:
      output += re[1][0] + " "
    ...
    #import model
    ent_list=[]
    nlp_ner = spacy.load("Videos/tkinter-app/model-best")
    doc = nlp_ner(output)
    for ent in doc.ents:
   ...
   #store name 
    ...
    if ores: 
      ...
      #database connection
      conn=mysql.connector.connect(host="localhost",username="root",password="kchaa",database="medicine")
      my_cursor=conn.cursor()

      #data query
      query = "SELECT * FROM test WHERE LOWER(DRUG_NAME) LIKE CONCAT('%', LOWER(%s), '%')"
      ...
      column_labels = [ "DRUG_NAME", "DOSAGE FORM AND STRENGTH", "INDICATIONS",
                 "CONTRAINDICATIONS OR PRECAUTIONS", "DOSAGE SCHEDULE", "ADVERSE EFFECTS", "DRUG AND FOOD INTERACTIONS"]
      ...
      text_widget.grid(row=2, column=1, columnspan=len(column_labels) + 1, pady=10, padx=10)
      ...
      l3 = tk.Label(new_window,text= "No entity found", font=Normal_font)  
      l3.grid(row=3,column =1 )
